...
namespace YoutubeClone.Membership.IdentityStores { 
    ...
    public class UserStore<TUser, TRole> :
        UserStoreBase<TUser, TRole, string, UserClaim, UserRole, UserLogin, UserToken, RoleClaim>,
        IProtectedUserStore<TUser> where TUser : ApplicationUser where TRole : Role
    {
        ...
        private IQueryable<UserClaim> UserClaims => session.Query<UserClaim>();

        private IQueryable<UserRole> UserRoles => session.Query<UserRole>();

        private IQueryable<UserLogin> UserLogins => session.Query<UserLogin>();

        private IQueryable<UserToken> UserTokens => session.Query<UserToken>();
